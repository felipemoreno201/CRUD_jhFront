...
describe('Autor Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'autor', component: AutorComponent }]), HttpClientTestingModule, AutorComponent],
      providers: [
        {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.autors?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to autorService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getAutorIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getAutorIdentifier).toHaveBeenCalledWith(entity);
